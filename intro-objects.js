...
var getFullNames = function(array) {
    ...
    for (var i = 0; i < array.length; i++) {
        nameArray.push(array[i].first + ' ' + array[i].last)
    }
    return nameArray
}
...
dog.speak = function(string) {
    var speakArray = string.split(' ')
    var newString = ''
    for (var i = 0; i < speakArray.length; i++) {
        if (i === 0) {
            newString += 'r' + speakArray[i].substring(1)
        }
        else {
            newString += ' r' + speakArray[i].substring(1)
        }
...
var getCounts = function(string) {
    ...
    var testArray = lowerString.split(' ')
    var counterObj = {}
    for(i = 0; i < testArray.length; i++) {
        if (!(counterObj.hasOwnProperty(testArray[i]))) {
            counterObj[testArray[i]] = 1
        }
        else {
            counterObj[testArray[i]]++
        }
    }
    return counterObj
...
var reverseAll = function(array) {
    var reversed = []
    for (var i = 0; i < array.length; i++) {
        reversed.push(reverseObject(array[i]))
    }
    return reversed
}
...
var objMatch = function(obj,search) {
    var objList = []
    var testList = []
    for (var prop in obj) {
    ...
    for (var prop in search) {
        testList.push(search[prop])
    }
    var matchCount = 0
    for (var i = 0; i < objList.length; i++) {
        for(var n = 0; n < testList.length; n++) {
            if (objList[i] === testList[n]) {
                matchCount++
            }
        }
    }
    if (matchCount === testList.length) {
        return obj
    }
}


var where = function(list,searchObj) {
    var returnList = []
    for (var i = 0; i < list.length; i++) {
        if (objMatch(list[i],searchObj) != null) {
            returnList.push(objMatch(list[i],searchObj))
        }
    }
    return returnList
}
...
politeObject.personalize = function(inputFunc) {
    var result = inputFunc
    return("Hi, my name is " + this.name + ", and the result is " + result + ".")
}
